...
export function ModelProviderIcon({
  ...
  const iconMap: Record<ModelProvider, string> = {
    dimatic: '/kortix-symbol.svg', // Dimatic modes use the Dimatic symbol
  ...
  // Special handling for Dimatic symbol - needs different invert behavior
  const isDimatic = provider === 'dimatic';
  ...
  return (
    ...
    >
      <Image
        ...
        className={cn(
          "object-contain",
          // Dimatic symbol: invert in dark mode (black symbol → white)
          // Other icons: invert in dark mode (black icons → white)
          isDimatic ? "dark:invert" : "dark:brightness-0 dark:invert"
        )}
