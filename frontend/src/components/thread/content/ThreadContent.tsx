                                                        ...
                                                        {groupIndex === finalGroupedMessages.length - 1 && !readOnly && streamingTextContent && (streamHookStatus === 'streaming' || streamHookStatus === 'connecting') && (
                                                            <div className="mt-2">
                                                                {(() => {
                                                                    ...
                                                                    const hasThinkingTag = streamingTextContent.includes('<think>');
                                                                    if (hasThinkingTag) {
                                                                        // If we have a closing tag, we might have more content after it
                                                                        const closingIndex = streamingTextContent.indexOf('</think>');

                                                                        if (closingIndex === -1) {
                                                                            // Open thinking tag - show loader
                                                                            // Also render any content BEFORE the thinking tag
                                                                            const thinkStartIndex = streamingTextContent.indexOf('<think>');
                                                                            const textBeforeThink = streamingTextContent.substring(0, thinkStartIndex);

                                                                            return (
                                                                                <>
                                                                                    {textBeforeThink && (
                                                                                        <ComposioUrlDetector content={textBeforeThink} className="text-sm prose prose-sm dark:prose-invert chat-markdown max-w-none [&>:first-child]:mt-0 prose-headings:mt-3 break-words overflow-wrap-anywhere" />
                                                                                    )}
                                                                        ...
                                                                        // If we have a closing tag, standard processing will handle stripping it via Markdown component
                                                                        // We just fall through to standard processing
                                                                    }
